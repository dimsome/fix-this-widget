...
function fixThisWidgetFeedbackEndpoint(): Plugin {
  ...
  return {
    ...
    configureServer(server) {
      server.middlewares.use('/api/fix-this-widget/feedback', async (request, response) => {
        const chunks: Buffer[] = [];
        ...
        const feedbackResponse = await handler(new Request('http://localhost/api/fix-this-widget/feedback', {
